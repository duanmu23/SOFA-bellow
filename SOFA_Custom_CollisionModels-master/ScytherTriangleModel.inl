...
ScytherTriangleCollisionModel<DataTypes>::ScytherTriangleCollisionModel()
    : d_bothSide(initData(&d_bothSide, false, "bothSide", "activate collision on both side of the triangle model"))
    , d_computeNormals(initData(
          &d_computeNormals, true, "computeNormals", "set to false to disable computation of triangles normal"))
    , m_mstate(nullptr)
    , m_topology(nullptr)
    , m_needsUpdate(true)
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::resize(int size)
{
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::init()
{
    ...
    m_mstate =
        dynamic_cast<sofa::core::behavior::MechanicalState<DataTypes>*>(this->getContext()->getMechanicalState());
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::updateNormals()
{
    for (int i = 0; i < size; i++)
    {
        Element                           t(this, i);
        const sofa::defaulttype::Vector3& pt1 = t.p1();
        const sofa::defaulttype::Vector3& pt2 = t.p2();
        const sofa::defaulttype::Vector3& pt3 = t.p3();
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::updateFromTopology()
{
    ...
    else
    {
        ...
        {
            sofa::core::topology::BaseMeshTopology::Triangle idx = m_topology->getTriangle(i);
            if (idx[0] >= npoints || idx[1] >= npoints || idx[2] >= npoints)
            {
                msg_error() << "Vertex index out of range in triangle " << i << ": " << idx[0] << " " << idx[1] << " "
                            << idx[2] << " ( total points=" << npoints << ")";
                if (idx[0] >= npoints)
                    idx[0] = npoints - 1;
                if (idx[1] >= npoints)
                    idx[1] = npoints - 1;
        ...
        {
            sofa::core::topology::BaseMeshTopology::Quad idx = m_topology->getQuad(i);
            if (idx[0] >= npoints || idx[1] >= npoints || idx[2] >= npoints || idx[3] >= npoints)
            {
                msg_error() << "Vertex index out of range in quad " << i << ": " << idx[0] << " " << idx[1] << " "
                            << idx[2] << " " << idx[3] << " ( total points=" << npoints << ")";
                if (idx[0] >= npoints)
                    idx[0] = npoints - 1;
                if (idx[1] >= npoints)
                    idx[1] = npoints - 1;
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::updateFlags(int /*ntri*/)
{
#if 0
    if (ntri < 0) ntri = m_triangles->size();
    //VecCoord& x =m_mstate->read(sofa::core::ConstVecCoordId::position())->getValue();
    //VecDeriv& v = m_mstate->read(sofa::core::ConstVecDerivId::velocity())->getValue();
...
template <class DataTypes>
bool ScytherTriangleCollisionModel<DataTypes>::canCollideWithElement(int index, CollisionModel* model2, int index2)
{
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::computeBoundingTree(int maxDepth)
{
    ...
    sofa::defaulttype::Vector3 minElem, maxElem;
    const VecCoord&            x = this->m_mstate->read(sofa::core::ConstVecCoordId::position())->getValue();
    ...
    if (!empty())
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            const sofa::defaulttype::Vector3& pt1 = x[t.p1Index()];
            const sofa::defaulttype::Vector3& pt2 = x[t.p2Index()];
            const sofa::defaulttype::Vector3& pt3 = x[t.p3Index()];
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::computeContinuousBoundingTree(double dt, int maxDepth)
{
    ScytherCubeModel* cubeModel = createPrevious<ScytherCubeModel>();
    ...
    if (!empty())
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            const sofa::defaulttype::Vector3& pt2  = t.p2();
            const sofa::defaulttype::Vector3& pt3  = t.p3();
            const sofa::defaulttype::Vector3  pt1v = pt1 + t.v1() * dt;
            const sofa::defaulttype::Vector3  pt2v = pt2 + t.v2() * dt;
            const sofa::defaulttype::Vector3  pt3v = pt3 + t.v3() * dt;
            ...
            for (int c = 0; c < 3; c++)
            {
                ...
                if (pt1v[c] > maxElem[c])
                    maxElem[c] = pt1v[c];
                else if (pt1v[c] < minElem[c])
                    minElem[c] = pt1v[c];
                if (pt2v[c] > maxElem[c])
                    maxElem[c] = pt2v[c];
                else if (pt2v[c] < minElem[c])
                    minElem[c] = pt2v[c];
                if (pt3v[c] > maxElem[c])
                    maxElem[c] = pt3v[c];
                else if (pt3v[c] < minElem[c])
                    minElem[c] = pt3v[c];
...
// template <class DataTypes>
// TriangleLocalMinDistanceFilter* ScytherTriangleCollisionModel<DataTypes>::getFilter() const
//{
//    return m_lmdFilter;
//}

// template <class DataTypes>
// void ScytherTriangleCollisionModel<DataTypes>::setFilter(TriangleLocalMinDistanceFilter* lmdFilter)
//{
//    m_lmdFilter = lmdFilter;
//}

template <class DataTypes>
int ScytherTriangleCollisionModel<DataTypes>::getTriangleFlags(Topology::TriangleID i)
{
    int                                              f = 0;
    ...
    if (i < m_topology->getNbTriangles())
    {
        ...
        {
            const sofa::core::topology::BaseMeshTopology::TrianglesAroundVertex& tav =
                m_topology->getTrianglesAroundVertex(t[j]);
        ...
        for (unsigned int j = 0; j < 3; ++j)
        {
            const sofa::core::topology::BaseMeshTopology::TrianglesAroundEdge& tae =
                m_topology->getTrianglesAroundEdge(e[j]);
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::computeBBox(const sofa::core::ExecParams* params, bool onlyVisible)
{
    if (!onlyVisible)
        return;
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::draw(const sofa::core::visual::VisualParams* vparams, int index)
{
    Element t(this, index);
...
template <class DataTypes>
void ScytherTriangleCollisionModel<DataTypes>::draw(const sofa::core::visual::VisualParams* vparams)
{
    if (vparams->displayFlags().getShowCollisionModels())
    {
        // In case topology has changed but drawing is called before the updateFromTopology has been computed, just exit
        // to avoid computation in drawing thread.
        if (m_topology->getRevision() != m_topologyRevision)
            return;
        ...
        vparams->drawTool()->setLightingEnabled(true);
        vparams->drawTool()->drawTriangles(points, indices, normals, sofa::defaulttype::Vec<4, float>(getColor4f()));
        vparams->drawTool()->setLightingEnabled(false);
        ...
        if (vparams->displayFlags().getShowNormals())
        {
            ...
            vparams->drawTool()->drawLines(points, 1, sofa::defaulttype::Vec<4, float>(1, 1, 1, 1));
        }
    }
